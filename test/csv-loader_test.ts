...
import { extractColumnMapping, loadCSV, ColumnType } from '../src/csv-loader';
...
describe('csv-loader', () => {
  describe('extractColumnMapping', () => {
    it('should map columns in any order', () => {
      ...
      expect(extractColumnMapping(locationSpec, ['id', 'latitude', 'longitude']))
        .to.deep.equal([
      ...
      expect(extractColumnMapping(locationSpec, ['id', 'longitude', 'latitude']))
        .to.deep.equal([
          { number: 0, name: 'id', type: ColumnType.STRING },
    ...
    it('should support optional columns', () => {
      ...
      expect(extractColumnMapping(optionalCol, ['a', 'b']))
        .to.deep.equal([
          { number: 0, name: 'a', type: ColumnType.STRING },
          { number: 1, name: 'b', type: ColumnType.STRING, optional: true },
        ]);

      expect(extractColumnMapping(optionalCol, ['a']))
        .to.deep.equal([
          { number: 0, name: 'a', type: ColumnType.STRING },
        ]);

      expect(() => extractColumnMapping(optionalCol, ['b'], 'filename'))
          .to.throw(/missing.*b.*filename/);
    });
    ...
    it('should map columns to destination fields', () => {
      ...
      return loadCSV<any>('./test/locations-sample.txt', spec).then(rows => {
